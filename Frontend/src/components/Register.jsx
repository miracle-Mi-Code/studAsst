...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Error occurred during registration setup.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-md w-full mx-auto mt-12 p-8 bg-white dark:bg-slate-900 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-800/80 card-animate">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-black text-slate-900 dark:text-white">Create Account</h2>
        <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm">
          Register to access student resources and GPA tracking
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4.5">
        ...
        <button 
          type="submit" 
          disabled={loading}
          className="w-full mt-4 py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg shadow-indigo-600/15 hover:shadow-indigo-600/25 transition-all duration-200 transform hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:pointer-events-none"
        >
          {loading ? 'Registering Account...' : 'Register Profile'}
        </button>
      </form>

      <div className="mt-6 pt-5 border-t border-slate-100 dark:border-slate-800/80 text-center text-sm text-slate-500 dark:text-slate-400">
